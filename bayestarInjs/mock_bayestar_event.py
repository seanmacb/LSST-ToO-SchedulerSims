...
def makeBayestarMock():
    ...
    outdir = "mock_simulations/"+trigger_id
    ...
    while True:
        ...
        print("Reading from {}/0.fits".format(outdir))
        if os.path.isfile(outdir+'/0.fits'):
            os.system("ligo-skymap-flatten --nside {} {} {}".format(2048, outdir+"/0.fits", outdir+"/0_flatten.fits"))
            ...
            area = np.sum(credible_levels <= 0.5) * hp.nside2pixarea(2048, degrees=True)
            ...
            if tries > allowedTries:
                print('Max number of bayestar sim attempts ({}) reached. Exiting.'.format(allowedTries))
                sys.exit(1)
            tries += 1
